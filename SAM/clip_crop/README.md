...
This project contains source code and supporting files for a serverless application for searching and cropping objects in image using OpenAI CLIP and Yolov5 in [PyTorch](https://pytorch.org/). It includes the following files and folders:
...
- app/weights - Pre-trained model weights
- app/yolov5 - Object detection model
...
```bash
clip_crop$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
clip_crop$ sam local start-api
clip_crop$ curl http://localhost:3000/clipcrop
```
...
```bash
clip_crop$ sam logs -n InferenceFunction --stack-name clip_crop --tail
```
...
```bash
aws cloudformation delete-stack --stack-name clip_crop
```
